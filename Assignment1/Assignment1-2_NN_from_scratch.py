...
for i, nn_hdim in enumerate(hidden_layer_dimensions):
    ...
    print("Loss with hidden size %i: %f" %(nn_hdim, loss))
...
def build_model_2(nn_hdim=10, mini_batch_size=1000, total_epoch=20001, print_loss=True):
    
    np.random.seed(0)
    # np.random.randn - normal distribution
    # why sqrt(nn_input_dim) ?
    W1 = np.random.randn(nn_input_dim, nn_hdim) / np.sqrt(nn_input_dim)
    # Why 1, nn_hdim ? => should be added via last dimension.
    b1 = np.zeros((1, nn_hdim))
    W2 = np.random.randn(nn_hdim, nn_output_dim) / np.sqrt(nn_hdim)
    b2 = np.zeros((1, nn_output_dim))
    ...
    for epoch in range(total_epoch):
        # mini-batch
        permutation = np.random.permutation(num_examples)[:mini_batch_size]
        mini_batch_train = train_dataset[permutation,:]
        mini_batch_labels = train_labels[permutation,:]
        
        # Forward propagation
        z1 = mini_batch_train.dot(W1) + b1
        
        a1 = np.tanh(z1)
        # ReLU
        #a1 = z1 * (z1 > 0).astype(np.float32)
        ...
        # Back propagation
        delta3 = probs - mini_batch_labels
        dW2 = (a1.T).dot(delta3)
        db2 = np.sum(delta3, axis=0, keepdims=True)
        
        delta2 = delta3.dot(W2.T) * (1 - np.power(a1, 2))
        # ReLU
        #delta2 = np.maximum((z1 > 0.0).astype(np.float32), np.zeros((1,nn_hdim))+1e-3)
        
        dW1 = np.dot(mini_batch_train.T, delta2)
        db1 = np.sum(delta2, axis=0)
        ...
        # Gradient descent parameter update
        step_lr = max(epsilon - 0.001*float(int(epoch/20)), 0.001)
        lr = step_lr
        ...
        if print_loss and epoch % 1000 == 0:
            print("Loss after iteration %i: %f" %(epoch, calculate_loss(model)))
            
    return model

model = build_model_2(nn_hdim=10, mini_batch_size=500, total_epoch=20001)
