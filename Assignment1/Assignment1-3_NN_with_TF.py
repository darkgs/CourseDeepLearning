...
with graph.as_default():
    ...
    w1 = tf.get_variable("W1", shape=[image_size * image_size, nn_hidden],
           initializer=tf.contrib.layers.xavier_initializer())
    b1 = tf.get_variable("B1", shape=[nn_hidden],
           initializer=tf.contrib.layers.xavier_initializer())
    w2 = tf.get_variable("W2", shape=[nn_hidden, num_labels],
           initializer=tf.contrib.layers.xavier_initializer())
    b2 = tf.get_variable("B2", shape=[num_labels],
           initializer=tf.contrib.layers.xavier_initializer())
...
# At the 0 step in the training phase, loss with naive (45.18) is very larger than layer module(2.30).
# Following the source code in the tensorflow. We found that weight and bias in the dense layer are initialized with xavier uniform using tf.contrib.keras.initializers.glorot_uniform function.
# https://www.tensorflow.org/versions/r1.1/api_docs/python/tf/contrib/keras/initializers/glorot_uniform
# 
# In this example, variables in the naive implementation initialize using random values from a truncated normal distribution as we can find in the document below.
# https://www.tensorflow.org/api_docs/python/tf/truncated_normal
# 
# So we did experiment to show that initialization effects the final accuracy. We initialized variables in the naive implementation using xavier uniform distribution. Final accuracy was 93.6% even better than the layer implementation.
# 
# In conclusion, better initialization effects the final accuracy.
...
graph_e=tf.Graph()
with graph_e.as_default():
    tf_dataset_e=tf.placeholder(tf.float32, shape=(None, image_size * image_size))
    tf_labels_e=tf.placeholder(tf.float32, shape=(None, num_labels))
    
    dense_1 = tf.layers.dense(tf_dataset_e, nn_hidden, activation=tf.nn.relu)
    dense_2 = tf.layers.dense(dense_1, nn_hidden, activation=tf.nn.relu)
    logits_e = tf.layers.dense(dense_2, num_labels, activation=tf.nn.softmax)
    
    #Loss
    loss_e = tf.reduce_mean(tf.nn.softmax_cross_entropy_with_logits_v2(labels=tf_labels_e, logits=logits_e))
    
    #Optimizer
    optimizer_e = tf.train.AdamOptimizer(1e-4).minimize(loss_e)
    
    #Predictions for the training
    prediction_e = tf.nn.softmax(logits_e)
    
with tf.Session(graph=graph_e, config=conf) as session_e:
    tf.global_variables_initializer().run()
    print("Initialized")
    for step in range(num_steps):
        offset = (step * batch_size) % (train_labels.shape[0] - batch_size)
        batch_data = train_dataset[offset:(offset + batch_size), :]
        batch_labels = train_labels[offset:(offset + batch_size), :].astype(float)
        feed_dict_e = {tf_dataset_e: batch_data, tf_labels_e: batch_labels}
        _, l_e, predictions_e = session_e.run([optimizer_e, loss_e, prediction_e], feed_dict=feed_dict_e)
        if(step % 1000 == 0):
            print("Minibatch loss at step %d: %f" % (step, l_e))
            feed_dict_val_e = {tf_dataset_e: valid_dataset}
            valid_prediction_e = session_e.run(prediction_e, feed_dict={tf_dataset_e: valid_dataset, tf_labels_e: valid_labels})
            print("Validation accuracy: %.1f%%" % accuracy(valid_prediction_e, valid_labels))

    feed_dict_test_e = {tf_dataset_e: test_dataset}
    test_prediction_e = session_e.run(prediction_e, feed_dict=feed_dict_test_e)
    print("Test accuracy: %.1f%%" % accuracy(test_prediction_e, test_labels))
