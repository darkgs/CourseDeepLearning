...
def rnn_step_backward(dnext_h, cache):
    ...
    delta = (1.0 - np.tanh(z))*(1.0 + np.tanh(z)) * dnext_h
    dx = np.dot(delta, np.transpose(Wx, (1,0)))
    dprev_h = np.dot(delta, np.transpose(Wh, (1,0)))
    dWx = np.dot(np.transpose(x, (1,0)), delta)
    dWh = np.dot(np.transpose(prev_h, (1,0)), delta)
...
def rnn_forward(x, h0, Wx, Wh, b):
    ...
    h = np.zeros((x.shape[1] + 1, x.shape[0], h0.shape[1]))
    z = np.zeros((x.shape[1], x.shape[0], h0.shape[1]))
    h[0] = h0
    for t in range(x_t.shape[0]):
        z[t] = np.dot(x_t[t], Wx) + np.dot(h[t], Wh) + b
        h[t+1] = np.tanh(z[t])

    h = np.transpose(h[1:,:,:], (1,0,2))
    z = np.transpose(z, (1,0,2))
    cache = (x, h0, h, Wx, Wh, z)
    ##########################################################################
    #                               END OF YOUR CODE                             #
    ##########################################################################
    return h, cache
...
def rnn_backward(dh, cache):
    ...
    - h, dh, z, delta (N, T, H)
    """
    x, h0, h, Wx, Wh, z = cache
    N = z.shape[0]
    T = z.shape[1]
    H = z.shape[2]
    D = x.shape[2]

    h_cut = np.concatenate((np.expand_dims(h0, axis=1), h), axis=1)[:,:-1,:]

    z_t = np.transpose(z, (1,0,2))
    dh_t = np.transpose(dh, (1,0,2))
    delta_t = np.zeros((T,N,H))
    for t in range(T):
        t = T - 1 - t
        delta_t[t] = (1.0 - np.tanh(z_t[t])) * (1.0 + np.tanh(z_t[t])) * dh_t[t]
        if t > 0:
            dh_t[t-1] += np.dot(delta_t[t], np.transpose(Wh, (1,0)))

    delta = np.transpose(delta_t, (1,0,2))

    dx = np.dot(delta, np.transpose(Wx, (1,0)))
    dh0 = np.dot(delta[:,0,:], np.transpose(Wh, (1,0)))
    dWx = np.dot(np.transpose(np.reshape(x, (N*T, D)), (1,0)), np.reshape(delta, (N*T, H)))
    dWh = np.dot(np.transpose(np.reshape(h_cut, (N*T, H)), (1,0)), np.reshape(delta, (N*T, H)))
    db = np.sum(np.sum(delta, axis=0), axis=0)
    ##########################################################################
    #                               END OF YOUR CODE                             #
    ##########################################################################
